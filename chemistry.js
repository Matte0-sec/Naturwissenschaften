(() => {
  ...
  function open(id) { selected = experiments[id]; values = { ...Object.fromEntries(selected.controls.map(([key,,,,value]) => [key, value])), material: "magnesium" }; time = 0; history = []; running = false; $("#chemistryEmpty").hidden = true; $("#chemistryPanel").hidden = false; $("#chemCategory").textContent = selected.category; $("#chemTitle").textContent = selected.title; $("#chemDescription").textContent = selected.description; $("#chemChallenge").textContent = selected.challenge; $("#chemControls").innerHTML = selected.controls.map(range).join("") + (id === "acidMaterials" ? `<label class="parameter material-select"><span>Material</span><select data-chem-material aria-label="Material in die Säure geben"><option value="magnesium">Magnesium</option><option value="zinc">Zink</option><option value="iron">Eisen</option><option value="copper">Kupfer</option></select></label>` : ""); document.querySelectorAll(".experiment-card[data-chem-id]").forEach((card) => card.classList.toggle("active", card.dataset.chemId === id)); document.querySelectorAll("[data-chem]").forEach((input) => input.addEventListener("input", () => { values[input.dataset.chem] = Number(input.value); const definition = selected.controls.find(([key]) => key === input.dataset.chem); $(`#chem-${input.dataset.chem}`).textContent = `${Number(input.value).toFixed(definition[4] % 1 ? 2 : 0)} ${definition[5]}`; render(); })); $("[data-chem-material]")?.addEventListener("change", (event) => { values.material = event.target.value; time = 0; history = []; render(); }); render(); }
  function calc() {
    ...
    if (id === "acidMaterials") {
      const materials = { magnesium: { name: "Magnesium", activity: 1, color: "#dfeee7" }, zinc: { name: "Zink", activity: .48, color: "#9aabb0" }, iron: { name: "Eisen", activity: .2, color: "#b66d4e" }, copper: { name: "Kupfer", activity: 0, color: "#e76f51" } };
