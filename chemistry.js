(() => {
  ...
  const data = [
    ["titration", "Säure-Base-Titration", "Analytik", "Eine starke Säure wird mit einer starken Lauge neutralisiert. Der pH-Sprung markiert den Äquivalenzpunkt.", "Wann schlägt der Indikator am deutlichsten um?", [["saeure", "Säurevolumen", 1, 50, 20, "mL"], ["lauge", "Laugenvolumen", 0, 50, 0, "mL"], ["konz", "Konzentration", .02, 1, .1, "mol/L"]]],
    ["kinetics", "Reaktionsgeschwindigkeit", "Kinetik", "Die Stoßtheorie: Höhere Temperatur und Konzentration erzeugen mehr wirksame Zusammenstöße.", "Wie wirkt sich eine Temperaturerhöhung auf die Reaktionszeit aus?", [["temperatur", "Temperatur", 10, 120, 25, "°C"], ["konz", "Konzentration", .05, 2, .5, "mol/L"], ["katalysator", "Katalysator", 0, 1, 0, "aus"]]],
    ["equilibrium", "Chemisches Gleichgewicht", "Gleichgewicht", "Hin- und Rückreaktion laufen gleich schnell. Konzentrationsänderungen verschieben das Gleichgewicht.", "Wie reagiert das System auf mehr Edukt?", [["edukt", "Eduktmenge", .1, 3, 1.5, "mol"], ["produkt", "Produktmenge", .1, 3, .5, "mol"], ["temperatur", "Temperatur", 10, 120, 25, "°C"]]],
    ["electrolysis", "Elektrolyse", "Elektrochemie", "Elektrische Ladung zerlegt eine Salzlösung. Faradays Gesetz verbindet Strom und abgeschiedene Stoffmenge.", "Wie viel Metall entsteht bei doppelter Stromstärke?", [["strom", "Stromstärke", .1, 10, 2, "A"], ["zeit", "Zeit", 1, 60, 20, "min"], ["spannung", "Spannung", 1, 24, 9, "V"]]],
    ["gases", "Ideales Gas", "Gase", "Druck, Volumen und Temperatur folgen näherungsweise der idealen Gasgleichung $pV=nRT$.", "Wie verändert sich der Druck bei halbem Volumen?", [["temperatur", "Temperatur", -50, 300, 20, "°C"], ["volumen", "Volumen", 1, 20, 8, "L"], ["stoffmenge", "Stoffmenge", .05, 2, .5, "mol"]]],
    ["calorimetry", "Kalorimetrie", "Thermochemie", "Bei einer Reaktion wird Energie frei oder aufgenommen. Die Lösung ändert ihre Temperatur.", "Wie verändert mehr Reaktionsstoff die Wärme?", [["stoff", "Stoffmenge", .01, 2, .4, "mol"], ["enthalpie", "Reaktionsenthalpie", -120, 120, -57, "kJ/mol"], ["wasser", "Wassermasse", 20, 1000, 200, "g"]]],
    ["solubility", "Löslichkeit", "Lösungen", "Die Löslichkeit vieler Feststoffe steigt mit der Temperatur. Überschuss bleibt als Bodensatz zurück.", "Ab wann kristallisiert der Überschuss aus?", [["temperatur", "Temperatur", 0, 100, 25, "°C"], ["salz", "Salzmenge", 1, 200, 80, "g"], ["wasser", "Wassermenge", 20, 500, 150, "mL"]]],
    ["redox", "Redox-Zelle", "Elektrochemie", "Zwei Halbzellen liefern eine Spannung. Die Potentialdifferenz treibt einen Elektronenstrom an.", "Welche Kombination erzeugt die höchste Zellspannung?", [["anode", "Anodenpotential", -2, 0, -.76, "V"], ["kathode", "Kathodenpotential", 0, 2, .34, "V"], ["konz", "Ionenkonzentration", .01, 2, 1, "mol/L"]]],
    ["molecules", "Molekülgeometrie", "Struktur", "Elektronenpaare stoßen sich ab. Das VSEPR-Modell sagt daraus Bindungswinkel voraus.", "Wie ändert ein freies Elektronenpaar die Geometrie?", [["bindungen", "Bindungspaare", 2, 4, 4, ""], ["freie", "Freie Elektronenpaare", 0, 2, 0, ""], ["bindung", "Bindungslänge", 40, 140, 80, "pm"]]],
  ...
  window.openChemistryPlatform=()=>{$("#homeScreen").hidden=true;$("#labScreen").hidden=true;$("#physicsPlatform").hidden=true;$("#chemistryPlatform").hidden=false;if(!$("#chemistryCards").children.length)build();}; $("#chemistryHomeButton").addEventListener("click",()=>{running=false;$("#chemistryPlatform").hidden=true;$("#homeScreen").hidden=false;}); $("#chemStart").addEventListener("click",()=>{running=true;}); $("#chemPause").addEventListener("click",()=>{running=false;}); $("#chemReset").addEventListener("click",()=>open(selected.id)); $("#chemistrySearch").addEventListener("input",(event)=>filter("",event.target.value)); requestAnimationFrame(tick);
})();